# Xiaomi MiMo Studio 去水印脚本 / Watermark Remover
...
1. 打开 Tampermonkey 管理面板 / Open the Tampermonkey dashboard
2. 点击"创建新脚本" / Click "Create a new script"
3. 将 `xiaomi-mimo-watermark-remover.user.js` 文件的内容复制到编辑器中 / Copy the contents of the `xiaomi-mimo-watermark-remover.user.js` file into the editor
4. 保存脚本（Ctrl+S 或 Cmd+S）/ Save the script (Ctrl+S or Cmd+S)
5. 确保脚本已启用 / Ensure the script is enabled
...
- **ENABLE_LOG**: 控制是否输出调试日志 / Controls whether to output debug logs
  - `false`（默认）：不输出日志，静默运行 / `false` (default): No logs output, runs silently
  - `true`：在浏览器控制台输出详细日志，便于调试 / `true`: Outputs detailed logs in the browser console for debugging
...
- ✅ Firefox
- ✅ Safari
- ✅ 其他支持 Tampermonkey 的浏览器 / Other browsers that support Tampermonkey

## 版本历史 / Version History

### v1.2.0
- 添加全局日志开关，默认关闭 / Added global log switch, disabled by default
- 统一日志输出格式 / Unified log output format

### v1.1.0
- 添加动态获取水印功能 / Added dynamic watermark fetching functionality
- 添加重试机制和页面检测备选方案 / Added retry mechanism and page detection fallback
- 改进错误处理和日志输出 / Improved error handling and log output

### v1.0.0
- 初始版本 / Initial release
- 支持多种水印形式的检测和移除 / Supports detection and removal of multiple watermark forms

## 注意事项 / Notes

- 本脚本仅用于学习和研究目的 / This script is for learning and research purposes only
- 使用前请确保遵守相关网站的使用条款 / Please ensure compliance with the relevant website's terms of use before using
- 脚本会自动获取当前登录用户的水印内容，无需手动配置 / The script automatically fetches the watermark content of the currently logged-in user, no manual configuration required
...
- [Tampermonkey 官网](https://www.tampermonkey.net/) / [Tampermonkey Official Website](https://www.tampermonkey.net/)
